...
class Employee :
    def __init__(self,root) :
        ...
        # logo
        img_logo = Image.open('D:/VS java/First/src/EMPLOYEEE_MANAGEMENT_SYSTEM/images/icon.jpg')
        img_logo = img_logo.resize((50,50))
        self.photo_logo = ImageTk.PhotoImage(img_logo)

        self.logo = Label(self.root,image=self.photo_logo)
        self.logo.place(x=280,y=1,width=50,height=50)

        #image frame
        img_frame = Frame(self.root,bd=2,relief='ridge',bg='white')
        img_frame.place(x=0,y=50,width=1550,height=300)

        # 1st image
        img1 = Image.open('D:/VS java/First/src/EMPLOYEEE_MANAGEMENT_SYSTEM/images/img1.png')
        img1 = img1.resize((430,423))
        self.photo1 = ImageTk.PhotoImage(img1)

        self.img_1 = Label(img_frame,image=self.photo1)
        self.img_1.place(x=0,y=0,width=500,height=200)

        # 2nd image
        img2 = Image.open('D:/VS java/First/src/EMPLOYEEE_MANAGEMENT_SYSTEM/images/img3.png')
        img2 = img2.resize((335,215))
        self.photo2 = ImageTk.PhotoImage(img2)

        self.img_2 = Label(img_frame,image=self.photo2)
        self.img_2.place(x=500,y=0,width=500,height=200)

        # 3rd image
        img3 = Image.open('D:/VS java/First/src/EMPLOYEEE_MANAGEMENT_SYSTEM/images/img2.png')
        img3 = img3.resize((373,180))
        self.photo3 = ImageTk.PhotoImage(img3)

        self.img_3 = Label(img_frame,image=self.photo3)
        self.img_3.place(x=1000,y=0,width=525,height=200)
        ...
        upper_frame.place(x=10,y=10,width=1440,height=240)
        ...
        lbl_combo.current(0)
        lbl_combo.grid(row=0,column=1,padx=2,pady=10,sticky=W)
        ...
        entry_name.grid(row=0,column=3,padx=2,pady=10,sticky=W)
        ...
        lbl_combo.current(0)
        lbl_combo.grid(row=3,column=3,padx=2,pady=10,sticky=W)
        ...
        lbl_id_type.grid(row=4,column=0,padx=2,pady=10,sticky=W)

        entry_id_proof =Entry(upper_frame,textvariable=self.idProof,font=('arial',11,'bold'),width=20)
        entry_id_proof.grid(row=4,column=1,padx=2,pady=10,sticky=W)


        #Add Employee Image
        employeeImg= Image.open('D:/VS java/First/src/EMPLOYEEE_MANAGEMENT_SYSTEM/images/employeeImage1.png')
        employeeImg= employeeImg.resize((110,110))
        self.employeePhoto = ImageTk.PhotoImage(employeeImg)

        self.imgEmp = Label(upper_frame,image=self.employeePhoto)
        self.imgEmp.place(x=900,y=0,width=110,height=110)


        #Add four button Update, Save, Delete, Reset

        button_frame = Frame(upper_frame,bd=2,relief='ridge',bg='white')
        button_frame.place(x=1100,y=4,width=125,height=210)

        btn_save = Button(button_frame,text="Save",command=self.AddData,font=('arial',12,'bold'),fg='white',bg='blue',width=12)
        btn_save.grid(row=0,column=0,pady=8,sticky=W)

        btn_update = Button(button_frame,text="Update",command=self.updateData,font=('arial',12,'bold'),fg='white',bg='blue',width=12)
        btn_update.grid(row=1,column=0,pady=8,sticky=W)

        btn_delete = Button(button_frame,text="Delete",command=self.deleteData,font=('arial',12,'bold'),fg='white',bg='blue',width=12)
        btn_delete.grid(row=2,column=0,pady=8,sticky=W)

        btn_reset = Button(button_frame,text="Reset",command=self.resetEmployeeInfo,font=('arial',12,'bold'),fg='white',bg='blue',width=12)
        btn_reset.grid(row=3,column=0,pady=8,sticky=W)

        # down frame
        down_frame = LabelFrame(Main_frame,bd=2,relief="ridge",bg='white',
                                 text='Employee Management Information Table',
                                 font=('arial',11,'bold'),fg='red')
        down_frame.place(x=10,y=250,width=1440,height=300)

        #searh Frame
        search_frame= LabelFrame(down_frame,text="Search Employee Information",relief='ridge',bd=2,font=('arial',11,'bold'),fg='red', bg='white')
        search_frame.place(x=5,y=0,width=1420,height=55)

        search_lbl = Label(search_frame,text="Search By:",font=('arial',12,'bold'),bg='red',fg='white')
        search_lbl.grid(row=0,column=0,padx=4,pady=5,sticky=W)
        ...
        table_frame.place(x=5,y=60,width=1420,height=180)
    ...
    def AddData(self):
        ...
        conn = mysql.connector.connect(host="localhost",user="root",password="Mysql",database ="employeeschema")
        cursor = conn.cursor()
        try:
            cursor.execute("insert into new_table values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(
                self.department.get(),
                self.name.get(),
                self.phone.get(),
                self.email.get(),
...
#delete data from database
    def deleteData(self):
        conn = mysql.connector.connect(host="localhost",username="root",password="Mysql",database="employeeschema")
        cursor= conn.cursor()
    
        try:
            cursor.execute("delete from new_table where IdProof= %s",(self.idProof.get(),))
            conn.commit()
...
#search Emoloyeeinfo
    def searchEmployeeInfo(self):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("select * from new_table where "+ str(self.searchBy.get()) + " LIKE %s",(self.searchText.get(),))
            data= cursor.fetchall()
...
#update data to database
    def updateData(self):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("update new_table set Department=%s,Name=%s,Phone=%s,Email=%s,DOB=%s,DOJ=%s,Salary=%s,IdProof=%s,IdType=%s,Married=%s,Gender=%s,Address=%s,Country=%s ",(
                self.department.get(),
                self.name.get(),
                self.phone.get(),
                self.email.get(),
